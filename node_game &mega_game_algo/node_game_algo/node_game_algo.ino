...
short D(short q, short l, short e, unsigned char E, unsigned char z, unsigned char n) {
  ...
  W(d++ < n || d < 3 ||                         /* iterative deepening loop */
    z & K == I && (N < T & d < 98 ||            /* root: deepen upto time   */ //d < 98
...
//*******************************   Check for invalid move punishment

bool isThereInvalidMovePunishment(){
   for (int i = 0; i < 8; i++) {
...
void AI_HvsC() {
  ...
  Serial.println("Thinking");                       /* Turn for ARDUINO */
  ...
  r = D(-I, I, Q, O, 1, 3);                 /* Check & do the human movement */ // n=3
  if ( !(r > -I + 1) ) {
    Serial2.println("ST");
     return;
    //gameOver();
  ...
  if(isThereInvalidMovePunishment()){

    Serial2.println("IP");
    }
  else{
        Serial2.print(lastM[0]);
        Serial2.print(lastM[1]);
        Serial2.print(lastM[2]);
        Serial2.print(lastM[3]);
        Serial2.print('\n');
        Serial.print("lastM sending: ");
